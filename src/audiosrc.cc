...
VirtFileSource::VirtFileSource (std::string const&)
	: SFSource ()
	, _offset (0)
	, _len (0)
	, _buf (0)
{
	static const uint8_t wavehead[] = {
		'R', 'I', 'F', 'F',
		0x34, 0x00, 0x00, 0x00, // SIZE = 4 + chunk-length == _len - 8 == 36 + raw-audio-bytes
		'W', 'A', 'V', 'E',

		'f', 'm', 't', ' ',
		0x10, 0x00, 0x00, 0x00, // Chunksize (16, 18 or 40)
		0x03, 0x00, // Format: 0x0003 = IEEE float
		0x01, 0x00, // Number of interleaved channels
		0x80, 0xbb, 0x00, 0x00, // Sample rate (48000)
		0x00, 0xee, 0x02, 0x00, // Data rate BytesPerSec (192000)
		0x04, 0x00, // Data block size (bytes)
		0x20, 0x00, // Bits per sample

		'd', 'a', 't', 'a',
		0x16, 0x00, 0x00, 0x00, // Payload size 32bytes, 4 samples
		/* payload follows */
	...
	_len = sizeof (wavehead);
	_buf = new uint8_t[_len];
	memcpy (_buf, wavehead, _len);

	open ();
	post_init ();
}

VirtFileSource::~VirtFileSource ()
{
	delete[] _buf;
}
...
VirtFileSource::open ()
{
	SF_VIRTUAL_IO sf_vio = {
		&VirtFileSource::sf_vio_get_filelen,
		&VirtFileSource::sf_vio_seek,
		&VirtFileSource::sf_vio_read,
		NULL,
		&VirtFileSource::sf_vio_tell
	};

	_sndfile = sf_open_virtual (&sf_vio, SFM_READ, &_info, this);
}

sf_count_t
VirtFileSource::sf_vio_get_filelen (void* user_data)
{
	return static_cast<VirtFileSource*> (user_data)->vio_get_filelen ();
}

sf_count_t
VirtFileSource::sf_vio_seek (sf_count_t offset, int whence, void* user_data)
{
	return static_cast<VirtFileSource*> (user_data)->vio_seek (offset, whence);
}

sf_count_t
VirtFileSource::sf_vio_read (void* ptr, sf_count_t count, void* user_data)
{
	return static_cast<VirtFileSource*> (user_data)->vio_read (ptr, count);
}

sf_count_t
VirtFileSource::sf_vio_tell (void* user_data)
{
	return static_cast<VirtFileSource*> (user_data)->vio_tell ();
}

sf_count_t
VirtFileSource::vio_get_filelen ()
{
	return _len;
}

sf_count_t
VirtFileSource::vio_seek (sf_count_t offset, int whence)
{
	...
	switch (whence) {
		...
		case SEEK_CUR:
			...
			break;
		case SEEK_END:
			new_offset = vio_get_filelen () + offset;
			break;
		default:
			return _offset;
			break;
	...
	if (new_offset >= 0 && new_offset < vio_get_filelen ()) {
		_offset = new_offset;
	}
...
sf_count_t
VirtFileSource::vio_tell ()
{
	return _offset;
}

sf_count_t
VirtFileSource::vio_read (void* ptr, sf_count_t count)
{
	sf_count_t remain = vio_get_filelen () - _offset;

	count = std::min (count, remain);

	if (count > 0) {
		memcpy (ptr, &_buf[_offset], count * sizeof (uint8_t));
		_offset += count;
	}
